...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (recipient.username && recipientId) {
      ...
      //jika route sekarang hanya /chat
      socket.disconnect();
      socket.auth = { username: loggedUser.username };
      socket.connect();
  ...
  messageBalloons = messages.map((chat, index) => {
    return <MessageBalloon key={index} message={chat?.message} username={chat?.from.username} timestamp={chat?.timestamp} />;
  });
  ...
  if (!recipientId) {
    lastMessages = contacts.map((lastmessage, index) => {
      return <LastMessage key={index} {...lastmessage} />;
    });
  }
    ...
    <section className="chat">
      {recipientId ? (
        <div className="row">
          <div className="chat-room-header">
            <div
              className="recipient"
              onClick={() => {
                history.push(`/profile/${recipient.id}`);
              }}
        ...
        <>
          ...
          {!isLoading ? (
            contacts.length ? (
              <div className="last-message-container">{lastMessages}</div>
            ) : (
